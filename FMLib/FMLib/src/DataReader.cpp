...
namespace FMLib
{
    ...
    void DataReader::LoadDataFromSlus(std::fstream& slus, InternalData& dat)
    {
        // General card data
        slus.seekg(0x1C4A44);
        for (int i = 0; i < 722; ++i)
        ...
        for(int i = 0; i < 39; ++i)
        {
            ...
            dat.gameData->Duelists[i].Name = new char[duelName.size() + 1]; // TODO: Check if any leaks occur
            strcpy_s(dat.gameData->Duelists[i].Name, duelName.size() + 1, duelName.c_str());
        }
    ...
    void DataReader::LoadDataFromWaMrg(std::fstream& mrg, InternalData& dat)
    {
        // Fusions
        mrg.seekg(0xB87800);
        unsigned char* fuseDat = new unsigned char[0x10000];
        ...
        for(int i = 0; i < 722; ++i)
        {
            ...
            if (position != 0)
            {
                ...
                while (num2 > 0)
                {
                    ...
                    auto fusion = dat.gameData->Cards[i].Fusions[fusionIndex];
                    fusion.Card1 = i + 1;
                    fusion.Card2 = num9 - 1;
                    ...
                    fusion = dat.gameData->Cards[i].Fusions[fusionIndex];
                    fusion.Card1 = i + 1;
        ...
        std::cout << "Finished loading fusions...\n";

        // Equips
        mrg.seekg(0xB85000);
        unsigned char* equipDat = new unsigned char[0x2800];
        ...
        std::cout << "Finished loading equips...\n";

        // Card costs/passwords
        unsigned char* starCost = new unsigned char[722 * 8];
        mrg.seekg(0xFB9808, mrg.beg);
        ReadType(mrg, starCost, 722 * 8);
        ...
        std::cout << "Finished loading starchips cost/password...\n";
        

        // Read duelist decks and card drops
        for(int i = 0; i < 39; ++i)
        {
            long num = 0xE9B000 + 0x1800 * i;
            mrg.seekg(num);

            unsigned char* memStream = new unsigned char[1444];
        ...
        std::cout << "Finished loading duelist decks and drops...\n";
    }

    void DataReader::LoadAllData(std::fstream& slus, std::fstream& mrg, InternalData& dat)
    {
