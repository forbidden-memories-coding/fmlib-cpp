...
namespace FMLib
{
    ...
    static void writeWithCrc(std::fstream& f, std::fstream& bin, int chunks)
    {
        ...
        {
            unsigned int crc = 0;
            f.read(crcCalc + 8, DATA_SIZE);

            for (char dataElement : crcCalc)
                crc = edcTable[(crc ^ dataElement) & 0xFF] ^ (crc >> 8);
            ...
            bin.seekg(24, bin.cur);
            bin.write(crcCalc + 8, DATA_SIZE);
            bin.write(crcCh, 4);
    ...
    bool DiscPatcher::PatchImage(const char* imgName)
    {
        ...
        const std::string cueSheet = fName + "  TRACK 01 MODE2/2352\n    INDEX 01 00:00:00\n";
        newCue.write(cueSheet.c_str(), cueSheet.size());
        newCue.close();
